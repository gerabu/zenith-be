...
Allow authenticated users with a connected Google Calendar to create bookings via `POST /bookings`, enforcing conflict detection against both existing internal bookings and live Google Calendar events before persisting.
