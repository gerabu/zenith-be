...
- **AND** the single `available` entry has no `title` field and no `id` field
...
- **THEN** the response timeline contains a `booked` slot for 09:00–10:00 with `title: "Dentist"` and the booking's `id`, an `external` slot for 14:00–15:00 with `title: "Team sync"` and no `id`, and `available` slots (without a title or `id`) for the remaining windows

#### Scenario: Booked entry exposes the internal booking id

- **WHEN** an authenticated user calls `GET /availability/2024-06-15` and has an internal booking on that date
- **THEN** the corresponding `booked` timeline entry includes an `id` field equal to that booking's `Booking.id`
